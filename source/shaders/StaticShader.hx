...
class StaticShader extends FlxShader // https://www.shadertoy.com/view/ldjGzV and https://www.shadertoy.com/view/Ms23DR and https://www.shadertoy.com/view/MsXGD4 and https://www.shadertoy.com/view/Xtccz4
{
  @:glFragmentSource('
    ...
    float snoise(vec2 v) {
        ...
        m = m * m * m * m;
        vec3 x = 2.0 * fract(p * vec3(1.0, 1.0, 1.0)) - 1.0;
        vec3 h = abs(x) - 0.5;
        vec3 ox = floor(x + 0.5);
        vec3 a0 = x - ox;
    ...
    void main() {
        ...
        for (float i = -1.0; i <= 1.0; i += 1.0) {
            float dist = i / 200.0;
            staticVal += staticV(uv + vec2(0.0, dist)) * (5.0 / 200.0 - abs(dist)) * 1.5;
        }
        staticVal *= bottomStaticOpt;

        vec4 texColor = flixel_texture2D(bitmap, vec2(uv.x + fuzzOffset + largeFuzzOffset * horzFuzzOpt, y));
        float red = texColor.r + staticVal;
        float green = texColor.g + staticVal;
        float blue = texColor.b + staticVal;
        float flAlpha = texColor.a + staticVal;
